...
def style_workbook(sheet, total_data):
    ...
    # Apply Border
    for row in sheet.iter_rows(min_row=1, max_row=total_data, min_col=1, max_col=4):
        for cell in row:
            if cell.row != 1 and cell.column != 2 and cell.column != 4:
...
def pdf_checking(pdf_file, properties, i):
    ...
    properties.append(
        [i+1, pdf_file.stem, pdf_document.page_count, str(pdf_file)])
...
def main():
    ...
    pdf_properties = []
    pdf_properties.append(['No', 'Name', 'Pages', 'Path'])
